...
- [x] **Stats / Insights screen.** New surface showing aggregate data: per-habit completion rate over the last 30 / 90 days (SwiftUI Charts line chart), per-goal contribution totals, longest streak per habit, total tasks completed, total gold earned, plots that have ever died (regret meter). Drives self-reflection and motivates streaks. Pure SwiftData queries — no schema change needed.
...
- [x] **Seasons.** Real-world calendar maps to spring / summer / fall / winter visuals. Farm scene tints + ambient particle effects change at each transition. Optional: per-season passive modifier (e.g. winter doubles decay for non-evergreens). `Season` enum derived from `Date`; no schema needed.
...
- [x] **Fix: weekly quest expiring mid-week.** `expireOldQuests` was treating `weeklyHarvest` quests (whose `day` = ISO week-start) the same as daily quests, causing them to expire the next day. Fixed: weekly quests compare against `thisWeekStart`, daily quests against `today`.
- [x] **Fix: replanting withered plots unavailable.** Plots remain `.withered` for up to 3 days before transitioning to `.dead`, but the replant button only appeared for `.dead` state — leaving users with no action at 0 health. Replant button now shows for both `.withered` and `.dead`; interactor guard updated to match. Section header reads "Plot is withering" vs "Plot died" accordingly.
- [x] **Feature: sort linked items incomplete-first.** Linked tasks and habits in `GoalDetailView` and `PlotDetailView` are now sorted so incomplete items float to the top.
- [x] **Feature: common field shows unlinked tasks and habits.** Tapping the common field plot now lists all unlinked habits (loggable) and incomplete unlinked tasks (completable) that feed it — same toggle-to-complete UX as goal plots.
- [x] **Feature: improved streak badge.** Replaced the small `🔥 N` caption text in habit rows with a dedicated trailing badge: `flame.fill` SF symbol + count in orange at subheadline weight. Shows from streak = 1 (was ≥ 2).
- [x] **Feature: vacation mode.** Settings > Farm toggle pauses daily plot health decay. The decay tick still advances so returning from a holiday doesn't cause a multi-day catch-up penalty against all plots.
- [x] **Feature: harvest / retire a completed goal's plot.** When all linked tasks are done (or the goal is manually set to Done), a "Goal complete!" banner appears at the top of the plot detail with a "Harvest plot (+N gold)" button. Harvesting awards gold scaled to plot health (base 30 + up to 20 bonus), detaches the goal (kept in goals list for records), and deletes the plot — freeing the capacity slot.
- [x] **Fix: harvested plot appeared as second Common Field.** Initial harvest implementation reset the plot to `.empty` instead of deleting it from the model context. Plot is now deleted on harvest so the capacity count and farm scene update immediately.
- [x] **Fix: harvest button required manual status change.** The harvest condition now fires automatically when all linked tasks are complete, without requiring the user to manually set the goal to Done status first.
